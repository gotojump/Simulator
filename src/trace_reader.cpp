...
trace_reader_t::trace_reader_t() {
	...
	this->trace_opcode_max = 0;				//get total opcodes 
};
...
bool trace_reader_t::trace_next_dynamic(uint32_t *next_bbl) {
	...
	*next_bbl = 0;
	...
	while (!valid_dynamic) {
		if (gzeof(this->gzDynamicTraceFile)) {		// Obtain the next trace line
			return FAIL;
		}
		...
		if (file_line[0] == '\0' || file_line[0] == '#') {		// Analyze the trace line
...
bool trace_reader_t::trace_next_memory(uint64_t *mem_address, uint32_t *mem_size, bool *mem_is_read){
	...
	while (!valid_memory) {
		if (gzeof(this->gzMemoryTraceFile)){		// Obtain the next trace line
			return FAIL;
		}
