...
template <class CB_TYPE>
int32_t circular_buffer_t<CB_TYPE>::push_back(const CB_TYPE& new_element){			// Insert into the newest position
	ERROR_ASSERT_PRINTF(this->data != NULL, "Trying to access beyond the circular buffer size.\n")
...
template <class CB_TYPE>
void circular_buffer_t<CB_TYPE>::pop_front(){			// Remove the oldest element
...
template <class CB_TYPE>
void circular_buffer_t<CB_TYPE>::pop_push(){			// Remove the oldest element and insert it into the newest position
...
template <class CB_TYPE>
void circular_buffer_t<CB_TYPE>::print_all(){			//Imprime todo o circular buffer, debug only
