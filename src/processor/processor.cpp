...
int32_t processor_t::searchPositionROB(){
	...
	if (this->robUsed < ROB_SIZE){		// There is free space.
...
void processor_t::rename(){
	...
	for(i = 0; i < RENAME_WIDTH; i++){
		...
		if (this->decodeBuffer.is_empty() || this->decodeBuffer.front()->status != PACKAGE_STATE_READY ||
